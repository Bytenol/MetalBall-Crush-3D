...
class Obstacle {
     ...
     */
    constructor(origin, pos, vertices, uvs, normals, isPath) {
        this.origin = origin;
        this.pos = pos;
     ...
     */
    static generateGeometryData(stride, scale, yTrans) {
...
const GameManager = (() => {
    const generatePoleBottomVertexData = (stride, scale, yTrans, topColor) => {
        let norm, col;
        let vertices = [];
        let colors = [];
        let normals = [];
    ...
    const initPoleVertexData = async (url) => {
...
const update = (dt) => {
    ...
    if (evt.pointerHeld) {
        // console.log(1);
        dynamicMesh.translation[1] += 0.2;
        staticMesh.translation[1] += 0.1;
...
const setupDOM = (gameAreaEl, gl, ctx) => {
 ...
 */
const transformVertices = (v, translate, scale) => {
    let sv = [];
 ...
 */
const repeatArray = (arrInp, l) => {
    let arr = [];
    for (let i = 0; i < l; i += arrInp.length)
        arr.push(...arrInp);
    return arr;
};
/**
...
const eventHandler = () => {
    addEventListener("keydown", e => {
        if(e.key === "a") evt.pointerHeld = true;
    });
